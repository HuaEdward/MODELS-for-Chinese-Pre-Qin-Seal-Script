...
class InscriptionPredictor:
    def __init__(self, model_path="best_model.pth"):
        ...
        初始化碑文预测器。
        Args:
            model_path (str): 训练好的分类器模型路径。
        """
        self.predictor = Predictor(model_path)

    def predict_inscription(self, inscription_images_dir):
        """
        预测整个碑文的国别归属，碑文由一个包含多个单字图像的文件夹表示。
        Args:
            inscription_images_dir (str): 包含单字图像的文件夹路径。
        Returns:
            dict: 包含各国概率的字典。
            list: 包含每个单字图像详细预测结果的列表。
        """
    ...
    def save_results(self, output_dir, inscription_name, overall_probabilities, detailed_results):
        """
        保存预测结果，包括一个总体概率图表和一个详细的CSV文件。
        Args:
            output_dir (str): 结果保存的目录。
            inscription_name (str): 碑文的名称，用于命名输出文件。
            overall_probabilities (dict): 碑文的总体预测概率。
            detailed_results (list): 每个单字图像的详细预测结果列表。
        """
