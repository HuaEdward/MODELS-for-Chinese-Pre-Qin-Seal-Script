...
class Predictor:
    ...
    def create_result_visualization(self, image_name, result, processed_img, output_dir):
        ...
        for bar, prob in zip(bars, probabilities):
            ...
            ax2.text(bar.get_x() + bar.get_width() / 2.0, height + 1, f'{prob:.1f}%', ha='center', va='bottom')
    ...
    def predict_batch(self, input_dir='prediction/input', output_dir='prediction/output'):
        ...
        for i, filename in enumerate(image_files, 1):
            ...
            if result is not None:
                ...
                chart_path = self.create_result_visualization(filename, result, processed_img, output_dir)
                processed_img_path = os.path.join(output_dir, f'{os.path.splitext(filename)[0]}_processed.png')
                cv2.imwrite(processed_img_path, processed_img)
                result_record = {'filename': filename, 'predicted_country': max_country, 'confidence': f'{max_prob:.1f}%', 'chart_path': os.path.basename(chart_path), 'processed_image_path': os.path.basename(processed_img_path)}
    ...
    while True:
        ...
        if choice == '1':
            ...
            if os.path.exists(image_path):
                ...
                if result:
                    ...
                    if save == 'y':
                        ...
                        chart_path = predictor.create_result_visualization(filename, result, processed_img, 'prediction/output')
            ...
            if os.path.exists(input_dir):
                input_files = [f for f in os.listdir(input_dir) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tiff'))]
